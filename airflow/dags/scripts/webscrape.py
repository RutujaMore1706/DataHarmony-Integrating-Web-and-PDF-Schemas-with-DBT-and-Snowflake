...
def main():
    ...
    url_1 = "https://www.cfainstitute.org"
    ...
    with open(csv_file_path, 'w', newline='', encoding='utf-8') as csvfile:
        ...
        for link in all_links:
            ...
            # Find all PDF links (those that end with .pdf)
            pdf_links = detailed_soup.find_all("a", href=lambda href: href and href.endswith('.pdf'))
            if pdf_links:
                pdf_link = pdf_links[0]['href']
                # Check if the href needs the base URL to be prepended
                if pdf_link.startswith('/-/'):
                    pdf_link = f"{url_1}{pdf_link}"
            else:
                pdf_link = "N/A"
